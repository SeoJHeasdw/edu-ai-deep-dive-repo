# Lab 02: Vector Database (ChromaDB)

## 🎯 학습 목표

임베딩과 Vector Database의 개념을 이해하고, ChromaDB를 사용하여 벡터 저장 및 유사도 검색을 실습합니다.

### 핵심 학습 내용

| 개념 | 설명 |
|------|------|
| **임베딩(Embedding)** | 텍스트를 숫자 벡터로 변환하는 기술 |
| **Vector DB** | 고차원 벡터를 효율적으로 저장하고 검색하는 데이터베이스 |
| **유사도 검색** | 벡터 간 거리/유사도를 기반으로 관련 문서 찾기 |
| **메타데이터 필터링** | 벡터 검색 + 조건 필터링 결합 |

## 📚 실습 항목

```
실습 1: 임베딩(Embedding) 이해하기
        └─ 텍스트가 어떻게 벡터로 변환되는지 직접 확인

실습 2: Vector DB 기본 작업
        └─ 문서 추가(Add) → 검색(Query) → 결과 해석

실습 3: 거리와 유사도 이해하기
        └─ L2 거리, 코사인 유사도의 의미와 해석 방법

실습 4: 메타데이터 필터링
        └─ 조건부 검색으로 정밀한 결과 얻기

실습 5: 실전 예제 - 문서 관리 시스템
        └─ 실제 애플리케이션 패턴 익히기
```

## 🚀 설치 및 실행

### ⚠️ Windows 사용자 주의사항

**Python 3.13에서 ChromaDB 사용 시 segmentation fault 발생 가능!**

#### 해결 방법 (우선순위 순)

**방법 1: Python 버전 다운그레이드 (권장)**
```bash
# pyenv 사용 (권장)
pyenv install 3.12.0
pyenv local 3.12.0

# 또는 conda 사용
conda create -n ai-labs python=3.12
conda activate ai-labs
```

**방법 2: 호환 버전 강제 설치**
```bash
pip install chromadb==0.4.22 hnswlib==0.8.0 --force-reinstall
```

**방법 3: WSL2 사용**
```bash
# Windows에서 WSL2 설치 후 Linux 환경에서 실행
wsl --install
```

### 일반 설치 및 실행

```bash
# 1. 프로젝트 루트로 이동
cd ai-basic-labs

# 2. 필요한 패키지 설치 (아직 안했다면)
pip install -r requirements.txt

# 3. 환경변수 설정 (.env 파일 생성)
echo "OPENAI_API_KEY=your-api-key-here" > .env

# 4. 실습 실행
cd lab02
python vector_db.py
```

### 문제 해결

**증상**: `Segmentation fault` 또는 프로그램이 갑자기 종료됨

**원인**: 
- Python 3.13과 ChromaDB의 의존성 라이브러리(hnswlib) 간 호환성 문제
- Windows에서 특히 빈번하게 발생

**확인 방법**:
```bash
python --version  # Python 버전 확인
pip list | grep chromadb  # ChromaDB 버전 확인
pip list | grep hnswlib   # hnswlib 버전 확인
```

**권장 환경**:
- Python: 3.11.x 또는 3.12.x
- ChromaDB: 0.4.22
- hnswlib: 0.8.0

## 📖 핵심 개념 설명

### 1. 임베딩(Embedding)이란?

```
텍스트 → 임베딩 모델 → 숫자 벡터

예시:
"Python is great" → [0.023, -0.156, 0.089, ..., 0.045]  (1536차원)
"코딩이 재미있어" → [0.031, -0.142, 0.095, ..., 0.038]  (1536차원)
```

**핵심 원리:**
- 의미가 비슷한 텍스트 → 비슷한 벡터
- 벡터 공간에서 가까운 위치에 배치
- 거리/유사도로 의미적 유사성 측정 가능

### 2. Vector DB vs 일반 DB

| 구분 | 일반 DB | Vector DB |
|------|---------|-----------|
| 검색 방식 | 정확한 값 매칭 | 유사도 기반 |
| 쿼리 예시 | `WHERE id = 123` | "가장 비슷한 벡터 찾기" |
| 용도 | 정확한 데이터 조회 | 의미 검색, 추천 시스템 |
| 인덱스 | B-tree, Hash | HNSW, IVF 등 ANN |

### 3. 거리 vs 유사도

```
┌─────────────────────────────────────────────────────────┐
│  L2 거리 (Euclidean Distance)                          │
│  • 값 범위: 0 ~ ∞                                      │
│  • 0에 가까울수록 = 더 유사                            │
│  • ChromaDB 기본 메트릭                                │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│  코사인 유사도 (Cosine Similarity)                      │
│  • 값 범위: -1 ~ 1                                     │
│  • 1에 가까울수록 = 더 유사                            │
│  • 텍스트 검색에서 많이 사용                           │
└─────────────────────────────────────────────────────────┘

거리 → 유사도 변환:
similarity = 1 / (1 + distance)
```

## 💻 주요 코드 예시

### 임베딩 생성

```python
from vector_db import EmbeddingGenerator

# 임베딩 생성기 초기화
embedder = EmbeddingGenerator()

# 단일 텍스트 임베딩
embedding = embedder.get_embedding("Hello, world!")
print(f"벡터 차원: {len(embedding)}")  # 1536

# 배치 임베딩 (효율적)
texts = ["Text 1", "Text 2", "Text 3"]
embeddings = embedder.get_embeddings_batch(texts)
```

### ChromaDB 기본 사용

```python
from vector_db import ChromaDBManager, EmbeddingGenerator

# 초기화
db = ChromaDBManager()
collection = db.create_collection("my_collection", reset=True)
embedder = EmbeddingGenerator()

# 문서 추가
documents = ["Document 1", "Document 2", "Document 3"]
embeddings = embedder.get_embeddings_batch(documents)

collection.add(
    documents=documents,
    embeddings=embeddings,
    ids=["doc_0", "doc_1", "doc_2"],
    metadatas=[{"source": "demo"} for _ in documents]
)

# 검색
query = "검색할 텍스트"
query_embedding = embedder.get_embedding(query)

results = collection.query(
    query_embeddings=[query_embedding],
    n_results=3
)
```

### 메타데이터 필터링

```python
# 단일 조건
results = collection.query(
    query_embeddings=[query_embedding],
    n_results=5,
    where={"language": "python"}
)

# 복합 조건 (AND)
results = collection.query(
    query_embeddings=[query_embedding],
    n_results=5,
    where={
        "$and": [
            {"language": "python"},
            {"level": "beginner"}
        ]
    }
)

# 비교 연산자
results = collection.query(
    query_embeddings=[query_embedding],
    n_results=5,
    where={"year": {"$gte": 2024}}  # 2024년 이후
)
```

### DocumentManager 사용 (실전 패턴)

```python
from vector_db import DocumentManager

# 문서 관리자 초기화
doc_manager = DocumentManager("knowledge_base", reset=True)

# 문서 추가
doc_id = doc_manager.add_document(
    text="Python은 배우기 쉬운 프로그래밍 언어입니다.",
    metadata={"topic": "programming", "language": "python"}
)

# 검색 (필터링 포함)
results = doc_manager.search(
    query="프로그래밍을 배우고 싶어요",
    n_results=5,
    where={"topic": "programming"}
)

for result in results:
    print(f"[{result.rank}] {result.content}")
    print(f"    유사도: {result.similarity:.4f}")
```

## 📊 필터 연산자 레퍼런스

| 연산자 | 의미 | 예시 |
|--------|------|------|
| `$eq` | 같음 | `{"field": "value"}` 또는 `{"field": {"$eq": "value"}}` |
| `$ne` | 같지 않음 | `{"field": {"$ne": "value"}}` |
| `$gt` | 크다 | `{"field": {"$gt": 10}}` |
| `$gte` | 크거나 같다 | `{"field": {"$gte": 10}}` |
| `$lt` | 작다 | `{"field": {"$lt": 10}}` |
| `$lte` | 작거나 같다 | `{"field": {"$lte": 10}}` |
| `$in` | 포함 | `{"field": {"$in": ["a", "b"]}}` |
| `$and` | 모두 만족 | `{"$and": [조건1, 조건2]}` |
| `$or` | 하나만 만족 | `{"$or": [조건1, 조건2]}` |

## 🔄 Vector DB 비교

| 특징 | ChromaDB | Pinecone | Qdrant | Milvus |
|------|----------|----------|--------|--------|
| 설치 | `pip install` | 클라우드 | Docker | Docker |
| 로컬 사용 | ✅ 매우 쉬움 | ❌ 불가 | ✅ 가능 | ✅ 가능 |
| 규모 | 소~중규모 | 대규모 | 대규모 | 대규모 |
| 비용 | 무료 | 유료 | 무료 (셀프) | 무료 (셀프) |
| 학습 난이도 | 쉬움 | 보통 | 보통 | 어려움 |

## 📁 생성되는 파일

```
lab02/
├── vector_db.py        # 실습 코드
├── README.md           # 이 파일
└── chroma_db/          # ChromaDB 데이터 저장소 (자동 생성)
    ├── chroma.sqlite3  # 메타데이터 저장
    └── ...             # 벡터 데이터
```

**주의**: `chroma_db/` 디렉토리를 삭제하면 모든 벡터 데이터가 초기화됩니다.

## 💡 학습 포인트 요약

### 1. 임베딩의 핵심
- 텍스트 → 고정 길이 숫자 벡터 (예: 1536차원)
- 의미가 비슷하면 벡터도 비슷
- OpenAI `text-embedding-3-small`: 성능과 비용의 균형

### 2. Vector DB의 역할
- 고차원 벡터의 효율적인 저장
- 빠른 유사도 검색 (ANN - Approximate Nearest Neighbor)
- 메타데이터와 함께 관리

### 3. 거리/유사도 해석
- L2 거리: 작을수록 유사 (0이 가장 가까움)
- 유사도: 클수록 유사 (1이 가장 비슷)
- 점수 해석은 상대적으로 판단

### 4. 메타데이터 활용
- 의미 검색 + 조건 필터링 결합
- 실무: 날짜, 카테고리, 권한 등 필터링
- 잘 설계된 메타데이터 = 유연한 검색

## ➡️ 다음 단계

- **Lab 03**: RAG (Retrieval-Augmented Generation) 시스템 구축
  - Vector DB를 활용한 문서 검색
  - 검색된 문서로 LLM 답변 생성
  - 청킹, 컨텍스트 관리, 고급 RAG 기법

## 📚 참고 자료

- [ChromaDB Documentation](https://docs.trychroma.com/)
- [OpenAI Embeddings Guide](https://platform.openai.com/docs/guides/embeddings)
- [Vector Database 개념](https://www.pinecone.io/learn/vector-database/)
- [Understanding Embeddings](https://towardsdatascience.com/getting-started-with-embeddings-a8f97bdf5a71)
